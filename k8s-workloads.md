  ...
  template:
    ...
    spec:
      ...
      - name: nginx
        ...
        - containerPort: 80
        env:
        - name: ENV_VAR_NAME      # Environment variable inside the container
          value: "production"
```

### Update strategies

**RollingUpdate (default)** — replaces pods gradually with zero downtime:
```yaml
spec:
  strategy:
    type: RollingUpdate
...
How it works:
- Creates 1 new pod (surge) → waits until ready → terminates 1 old pod → repeats
- Zero downtime — always has enough running pods
...
How it works:
- All existing pods are terminated at once
- New pods are created after all old ones are gone
- Has downtime during the transition
- Use when: old and new versions cannot run at the same time (e.g. database schema changes)

### Rollout commands — complete reference
```bash
# Apply a deployment
kubectl apply -f nginx-deployment.yaml

# Check rollout progress (wait until complete)
...
# Pause a rollout mid-way (freeze the update)
...
### StatefulSet — node failure scenarios

What happens to a StatefulSet pod when its node goes down?

**Scenario A — StatefulSet WITH PVC (volumeClaimTemplates)**

```
Node1 dies → db-0 pod becomes Terminating/Unknown
           → Kubernetes reschedules db-0 to Node2
           → db-0 on Node2 REATTACHES to the SAME PVC (db-data-db-0)
           → Data is preserved ✅
```

Kubernetes guarantees that `db-0` always claims `db-data-db-0`, `db-1` always claims `db-data-db-1`, etc. The PVC follows the pod identity — not the physical node.

**Scenario B — StatefulSet WITHOUT PVC (using emptyDir)**

```
Node1 dies → db-0 pod becomes Terminating/Unknown
           → Kubernetes reschedules db-0 to Node2
           → emptyDir on Node2 is EMPTY — data from Node1 is gone ❌
```

Never use `emptyDir` in a StatefulSet if your data must survive pod restarts or node failures.

### Volume types — when to use what

| Volume Type | Survives pod restart? | Survives node failure? | Shared across pods? |
|------------|----------------------|----------------------|---------------------|
| `emptyDir` | No (deleted with pod) | No | No (per-pod) |
| `hostPath` | Yes (if pod re-lands same node) | No | No |
| PVC (RWO) | Yes | Yes (reattaches) | No |
| PVC (RWX / NFS) | Yes | Yes | Yes |
| `local` PV | Yes (node affinity pins pod) | Only if node recovers | No |

**Key rule**: StatefulSet → always use `volumeClaimTemplates` with a proper StorageClass. Never use `emptyDir` if the data matters. — one Pod per node
...
spec:
  ...
  ttlSecondsAfterFinished: 60  # Auto-delete Job + its pods after 60 seconds
  template:
    spec:
      containers:
      - name: processor
...
`ttlSecondsAfterFinished`: automatically cleans up the Job (and its pods) after the specified seconds. Without this, finished Jobs stay and clutter `kubectl get jobs` output.
    ...
    spec:
      template:
        spec:
          ...
          - name: backup-tool
            image: busybox:1.28
            command: ["sh", "-c", "echo Backup completed at $(date); sleep 5"]
          restartPolicy: OnFailure  # Retry on failure
```

`successfulJobsHistoryLimit` / `failedJobsHistoryLimit`: controls how many old Job records are retained by the CronJob controller. Keeps your cluster clean — defaults are 3 for successful, 1 for failed.
...
### What are Labels?
Labels are key-value pairs attached to any Kubernetes object (pods, nodes, services, etc.). They are used to identify and organize objects.
...
Labels are not unique — many pods can have `app: nginx`. That's intentional: selectors use labels to find groups of pods.

### What are Selectors?
Selectors filter resources by their labels. There are two types:

**Equality-based (matchLabels)** — simple key=value matching:
```yaml
selector:
  matchLabels:
    app: nginx       # Must have exactly this label
    env: production  # AND this label
```

**Set-based (matchExpressions)** — more flexible operators:
```yaml
selector:
  matchExpressions:
  - key: app
    operator: In           # app must be one of the listed values
    values: ["nginx", "apache"]
  - key: env
    operator: NotIn        # env must NOT be any of the listed values
    values: ["test"]
  - key: version
    operator: Exists       # label 'version' must exist (any value)
```
...
### Deployment with selector example
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    metadata:
      labels:
        app: nginx           # Pods MUST have this label to match above selector
    spec:
      containers:
      - name: nginx
        image: nginx:latest
...
The `selector` field MUST match the `template.metadata.labels`. If they don't match, the Deployment controller won't manage those pods.

### Common label commands
```bash
# Show labels on pods
...
# Remove a label from a pod (dash at end)
...
ResourceQuota limits total CPU, memory, pods, PVCs, and other resources in a namespace. Without it, one team can consume all cluster resources.
...
spec:
  hard:
    requests.cpu: "2"              # Total CPU requested by all pods in namespace
    requests.memory: "4Gi"        # Total memory requested by all pods
    limits.cpu: "4"                # Total CPU limit across all pods
    limits.memory: "8Gi"          # Total memory limit across all pods
    pods: "10"                     # Max number of pods in this namespace
    persistentvolumeclaims: "5"   # Max number of PVCs in this namespace
```

Troubleshooting ResourceQuota exhaustion:
```bash
# See quota usage (used vs allowed)
kubectl describe resourcequota team-quota -n dev
# Output includes:
# Resource                Used  Hard
# --------                ----  ----
# pods                    10    10       ← FULL, new pod will be rejected
# persistentvolumeclaims  3     5

# See quota summary on the namespace itself
kubectl describe ns dev

# Error message you'll see when creating a pod over quota:
# Error from server (Forbidden): pods "new-pod" is forbidden:
# exceeded quota: team-quota, requested: pods=1, used: pods=10, limited: pods=10
```

### RBAC per namespace — restrict who can do what

Create a Role (namespaced) and bind it to a specific user:
...
metadata:
  name: dev-role
  namespace: dev             # This Role only applies inside the dev namespace
rules:
...
subjects:
- kind: User
  name: john-doe             # The user this binding applies to
  apiGroup: rbac.authorization.k8s.io
roleRef:
  kind: Role
  name: dev-role             # References the Role above
  apiGroup: rbac.authorization.k8s.io
```

Verify: can john-doe list pods in dev?
```bash
kubectl auth can-i list pods --as=john-doe -n dev
# yes

kubectl auth can-i delete deployments --as=john-doe -n prod
# no (binding is only in dev namespace)
...
spec:
  ...
  - name: app
    ...
    lifecycle:
      postStart:
        exec:
          command: ["/bin/sh", "-c", "echo Container started > /tmp/lifecycle.log"]
      preStop:
        exec:
          command: ["/bin/sh", "-c", "echo Stopping >> /tmp/lifecycle.log && sleep 10"]
...
### 5. ExternalName — map a service to an external DNS name
- Maps a Service to an external DNS name (outside the cluster)
- Useful when your app inside the cluster needs to talk to an external service (e.g. a managed database)
- No selector needed — Kubernetes resolves the DNS and returns a CNAME record
...
```bash
# Pods inside the cluster use the service name
curl http://my-db-service.default.svc.cluster.local
# Kubernetes DNS resolves this to db.example.com
```

Use case: You have a managed RDS database at `db.example.com`. Rather than hardcoding that URL in every app, you create an ExternalName service. If the external URL changes, you only update the service, not every app config.
...
| `nodePort` | Port on the Node's IP (NodePort/LoadBalancer only, range 30000-32767) |
...
> **Killercoda prerequisite:** Metrics Server is NOT pre-installed. Run this first:
> ```bash
> kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml
> # Patch to allow insecure TLS (needed in Killercoda kubeadm cluster)
...
### HPA behavior — control scale-up and scale-down speed

By default, HPA can scale down aggressively (removing pods too fast when load drops). Use `behavior` to add a stabilization window:
...
spec:
  ...
  behavior:
    scaleDown:
      ...
      # Prevents flapping: don't scale down immediately after a brief traffic dip
    scaleUp:
      stabilizationWindowSeconds: 0     # Scale up immediately when load hits
```

Why `stabilizationWindowSeconds: 300` for scaleDown:
- Prevents "flapping" — where pods are repeatedly added and removed as load oscillates
- Gives your app time to handle the burst before declaring it's safe to scale down
- 300s (5 min) is a common production default

### Load test — how to trigger HPA scaling in the lab

```bash
# Start a temporary busybox pod and hammer the service with requests
kubectl run test-box --rm -it --image=busybox -- /bin/sh

# Inside the shell — infinite loop sending requests
...
```bash
kubectl get hpa hpa-nginx --watch
# You'll see TARGETS go from 2%/50% → 80%/50% → new pods created
```

### Create HPA imperatively (exam shortcut)
```bash
# Create HPA directly from a deployment (without YAML)
kubectl autoscale deployment hpa-nginx --min=1 --max=5 --cpu-percent=50
``` — auto-scaling resource requests
...
spec:
  containers:
  - name: app
    image: heavy-java-app:latest  # conceptual — replace with your slow-start app
    startupProbe:
      httpGet:
        path: /healthz
        port: 8080
